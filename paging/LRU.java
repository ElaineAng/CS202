...
public class LRU {
	...
	Reference ref;
	boolean v;
	public LRU(int frameNum, int pageSize, int processSize, int jobMix, int referenceNum, boolean verbose){
		fn = frameNum;
		pas = pageSize;
		ps = processSize;
		jm = jobMix; 
...
//		}
		...
		if (where == -1){	//page fault
			...
			if (frames.size() == fn){	//does not have empty, require eviction
				int[] removed = frames.removeLast(); 	//remove the lru, use its frame number as the number for new frame,
			...
			for (int i=0; i<frames.size(); i++){
				int[] f = frames.get(i);
				if (f[0] == where){
					int[] toMove = frames.remove(i);
					frames.addFirst(toMove);
					break;
				}
			}		
		}
